...
class UserKeyTestCase(TestCase):
    ...
    def setUp(self):
        ...
        for username in ['alice', 'bob']:
            ...
            self.TEST_KEYS['{}_public'.format(
                username)] = key.publickey().exportKey('PEM')
            self.TEST_KEYS['{}_private'.format(
                username)] = key.exportKey('PEM')
    ...
    def test_02_activate(self):
        ...
        alice_uk = UserKey(user=User.objects.get(
            username='alice'), public_key=self.TEST_KEYS['alice_public'])
        self.assertFalse(alice_uk.is_active(),
                         "Inactive UserKey is_active() did not return False")
        alice_uk.activate(master_key)
        self.assertTrue(alice_uk.is_active(),
                        "ActiveUserKey is_active() did not return True")
    ...
    def test_03_key_sizes(self):
        ...
        """
        rsa = RSA.generate(
            getattr(settings, 'SECRETS_MIN_PUBKEY_SIZE', 2048) - 256)
    ...
    def test_04_master_key_retrieval(self):
        ...
        alice_uk = UserKey(user=User.objects.get(
            username='alice'), public_key=self.TEST_KEYS['alice_public'])
        alice_uk.activate(master_key)
        retrieved_master_key = alice_uk.get_master_key(
            self.TEST_KEYS['alice_private'])
        self.assertEqual(master_key, retrieved_master_key,
...
class SecretTestCase(TestCase):
    ...
    def test_01_encrypt_decrypt(self):
        ...
        self.assertIsNone(
            s.plaintext, "Plaintext must be None after encrypting.")

        # Enforce minimum ciphertext length
        self.assertGreaterEqual(len(
            s.ciphertext), 80, "Ciphertext must be at least 80 bytes (16B IV + 64B+ ciphertext")

        # Ensure proper hashing algorithm is used
        hasher, iterations, salt, sha256 = s.hash.split('$')
        self.assertEqual(hasher, 'pbkdf2_sha256',
        ...
        self.assertTrue(s.validate(plaintext),
    ...
    def test_02_ciphertext_uniqueness(self):
        ...
        duplicate_ciphertexts = [i for i, x in enumerate(
            ciphertexts) if ciphertexts.count(x) > 1]
        self.assertEqual(duplicate_ciphertexts, [],
